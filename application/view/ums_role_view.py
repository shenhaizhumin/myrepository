...
async def create_cole(schema: UmsRoleSchema, db=Depends(get_db), user_data=Depends(verify_token)):
...
async def list_menu(role_id=Path(..., alias='roleId'), db=Depends(get_db), user_data=Depends(verify_token)):
    result = await role_service.list_menu(db, role_id)
    return BaseResponse.success(result)


@ums_role_router.get('/listResource/{roleId}', summary='获取角色相关资源')
async def list_resource(role_id=Path(..., alias='roleId'), db=Depends(get_db), user_data=Depends(verify_token)):
